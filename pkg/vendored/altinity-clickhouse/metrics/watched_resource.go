...
func NewWatchedCR(src v1.ICustomResource) *WatchedCR {
	cr := &WatchedCR{}
	cr.readFrom(src)
	return cr
}

func (cr *WatchedCR) readFrom(src v1.ICustomResource) {
	if cr == nil {
		return
	}
	...
	src.WalkClusters(func(cl v1.ICluster) error {
...
func (cluster *WatchedCluster) readFrom(c v1.ICluster) {
	if cluster == nil {
		return
	}
	cluster.Name = c.GetName()

	c.WalkHosts(func(h *v1.Host) error {
		host := &WatchedHost{}
		host.readFrom(h)
...
func (host *WatchedHost) readFrom(h *v1.Host) {
	if host == nil {
		return
	}
	host.Name = h.Name
