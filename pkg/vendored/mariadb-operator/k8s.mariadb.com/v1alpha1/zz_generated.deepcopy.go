...
import (
	...
	"github.com/wandb/operator/pkg/vendored/mariadb-operator/k8s.mariadb.com"
...
func (in *GaleraRecoveryStatus) DeepCopyInto(out *GaleraRecoveryStatus) {
	*out = *in
	if in.State != nil {
		...
		*out = make(map[string]*k8s_mariadb_com.GaleraState, len(*in))
		for key, val := range *in {
			var outVal *k8s_mariadb_com.GaleraState
			if val == nil {
				(*out)[key] = nil
			} else {
				inVal := (*in)[key]
				in, out := &inVal, &outVal
				*out = new(k8s_mariadb_com.GaleraState)
				**out = **in
			}
			(*out)[key] = outVal
		}
	...
	if in.Recovered != nil {
		...
		*out = make(map[string]*k8s_mariadb_com.Bootstrap, len(*in))
		for key, val := range *in {
			var outVal *k8s_mariadb_com.Bootstrap
			if val == nil {
				(*out)[key] = nil
			} else {
				inVal := (*in)[key]
				in, out := &inVal, &outVal
				*out = new(k8s_mariadb_com.Bootstrap)
				**out = **in
			}
			(*out)[key] = outVal
		}
