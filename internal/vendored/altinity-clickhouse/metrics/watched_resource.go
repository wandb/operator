...
func NewWatchedCR(src v2.ICustomResource) *WatchedCR {
	cr := &WatchedCR{}
	cr.readFrom(src)
	return cr
}

func (cr *WatchedCR) readFrom(src v2.ICustomResource) {
	if cr == nil {
		return
	}
	...
	src.WalkClusters(func(cl v2.ICluster) error {
...
func (cluster *WatchedCluster) readFrom(c v2.ICluster) {
	if cluster == nil {
		return
	}
	cluster.Name = c.GetName()

	c.WalkHosts(func(h *v2.Host) error {
		host := &WatchedHost{}
		host.readFrom(h)
...
func (host *WatchedHost) readFrom(h *v2.Host) {
	if host == nil {
		return
	}
	host.Name = h.Name
