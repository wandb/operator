...
const (
	...
	// watchtowerAppName names the Application, and therefore the Deployment and
	// Service the application controller derives from it. Prefixed so it cannot
	// collide with an application the server manifest may publish later.
	watchtowerAppName = "wandb-watchtower"
...
func reconcileWatchtower(
	...
	desired := buildWatchtowerApplication(wandb, authService)

	application := &apiv2.Application{
		ObjectMeta: metav1.ObjectMeta{
			Name:      watchtowerAppName,
	...
	logger.Info(fmt.Sprintf("Successfully %s Watchtower Application", op),
		"application", watchtowerAppName, "authService", authService)

	wandb.Status.WatchtowerStatus = &apiv2.WatchtowerStatusSummary{
		Ready:       application.Status.Ready,
		URL:         watchtowerURL(wandb),
		Image:       wandb.Spec.Watchtower.GetImage(wandb.Spec.Global.ImageRegistry),
...
func deleteWatchtower(ctx context.Context, c ctrlClient.Client, wandb *apiv2.WeightsAndBiases) error {
	objects := []ctrlClient.Object{
		&apiv2.Application{ObjectMeta: metav1.ObjectMeta{Name: watchtowerAppName, Namespace: wandb.Namespace}},
		&rbacv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: watchtowerAppName, Namespace: wandb.Namespace}},
		&rbacv1.Role{ObjectMeta: metav1.ObjectMeta{Name: watchtowerAppName, Namespace: wandb.Namespace}},
...
func buildWatchtowerApplication(wandb *apiv2.WeightsAndBiases, authService string) *apiv2.Application {
					...
					Containers: []corev1.Container{
						{
							...
							Image:           watchtower.GetImage(wandb.Spec.Global.ImageRegistry),
...
func watchtowerServiceAccountName(wandb *apiv2.WeightsAndBiases) string {
	...
	return apiv2.DefaultWatchtowerServiceAccountName
}
...
func reconcileWatchtowerRBAC(ctx context.Context, c ctrlClient.Client, wandb *apiv2.WeightsAndBiases) error {
	...
	role := &rbacv1.Role{ObjectMeta: metav1.ObjectMeta{Name: watchtowerAppName, Namespace: wandb.Namespace}}
	...
	roleBinding := &rbacv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: watchtowerAppName, Namespace: wandb.Namespace}}
