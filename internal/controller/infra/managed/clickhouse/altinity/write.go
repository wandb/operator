...
func WriteState(
	...
	if clickHouseFound {
		actualLayout := actualClickHouse.Spec.Configuration.Clusters[0].Layout
		desiredLayout := desired.Spec.Configuration.Clusters[0].Layout
		topologyChanged = actualLayout.ShardsCount != desiredLayout.ShardsCount ||
			actualLayout.ReplicasCount != desiredLayout.ReplicasCount
	}
	if topologyChanged {
