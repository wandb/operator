...
func ComputeStatus(
	...
	connection *translator.RedisConnection,
	currentGeneration int64,
) (translator.RedisStatus, []corev1.Event, ctrl.Result) {
	ctx, _ = logx.WithSlog(ctx, logx.Redis)
	result := translator.RedisStatus{}
...
func inferInfraState(
	...
	if impliedStates[RedisConnectionInfoType] == common.UnavailableState &&
		...
		events = append(events, corev1.Event{
			...
			Message: "Redis connection info is unavailable, but Redis is reported as ready.",
		})
